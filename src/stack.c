...
//I want to store the "next" from the node, erase the node using free, I think. then use the "next" address to get to the next node and repeat until the value of next is NULL, but I cant' remember the syntax for doing that.
    if (!st){
        return;
    }
    if (*st){
